...
DeerFlow has its own detailed CLAUDE.md files at `deer-flow/backend/CLAUDE.md` (31KB) and `deer-flow/frontend/CLAUDE.md` (4.4KB). Refer to those for deep backend/frontend guidance.
...
- `${HOME}/.claude` and `${HOME}/.codex` bind mounts cause `mkdir Access is denied` — remove from volumes
...
See `life-os/CLAUDE.md` for detailed guidance.
...
- **Skills**: Shared between DeerFlow (`deer-flow/skills/public/`) and Claude Code (`~/.claude/skills/`)
